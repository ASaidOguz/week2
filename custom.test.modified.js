...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
   ...
   // Alice withdraws 0.08 ETH from the shielded pool to a recipient in L2
   const aliceWithdrawAmount = utils.parseEther('0.08')
   const recipient = ethers.utils.getAddress("0x8ba1f109551bd432803012645ac136ddd64dba72");               // '0xDeaD00000000000000000000000000000000BEEf'
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
      ...
      //Bob withdraws all his funds in L2 -> Alice withdraws all her remaining funds in L1 
    ...
    const bobEthAddress =ethers.utils.getAddress("0x8ba1f109551bd432803012645ac136ddd64dba72");                                                    
    ...
    const aliceChangeUtxo1 = new Utxo({
      amount: (aliceDepositAmount.sub(aliceWithdrawAmount)).sub(bobSendAmount),
      keypair: aliceKeypair,
    })
    await transaction({
      tornadoPool,
